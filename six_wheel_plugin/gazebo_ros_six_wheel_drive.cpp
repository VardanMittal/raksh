...
#include <gazebo_plugins/gazebo_ros_six_wheel_drive.h>
...
namespace gazebo
{
    ...
    GazeboRosSixWheelDrive::GazeboRosSixWheelDrive() {}
    ...
    // Load the controller
    void GazeboRosSixWheelDrive::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
    {
        ...
        {
            ROS_INFO_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin missing <robotNamespace>, defaults to \"%s\"",
        ...
        if (!_sdf->HasElement("broadcastTF"))
        {
            if (!this->broadcast_tf_)
                ROS_INFO_NAMED("siz_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <broadcastTF>, defaults to false.", this->robot_namespace_.c_str());
            else
                ROS_INFO_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <broadcastTF>, defaults to true.", this->robot_namespace_.c_str());
        }
        else
        {
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <leftFrontJoint>, defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <rightFrontJoint>, defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <leftRearJoint>, defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <rightRearJoint>, defaults to \"%s\"",
        ...
        this->right_middle_joint_name_ = "right_middle_joint";
        if (!_sdf->HasElement("rightMiddleJoint"))
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <rightMiddleJoint>, defaults to \"%s\"",
                           this->robot_namespace_.c_str(), this->right_middle_joint_name_.c_str());
        }
        else
        {
            this->right_rear_joint_name_ = _sdf->GetElement("rightMiddleJoint")->Get<std::string>();
        }
        this->right_rear_joint_name_ = "left_middle_joint";
        if (!_sdf->HasElement("leftMiddleJoint"))
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <leftMiddleJoint>, defaults to \"%s\"",
                           this->robot_namespace_.c_str(), this->left_middle_joint_name_.c_str());
        }
        else
        {
            this->left_middle_joint_name_ = _sdf->GetElement("leftMiddleJoint")->Get<std::string>();
        }
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <wheelSeparation>, defaults to value from robot_description: %f",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <wheelDiameter>, defaults to %f",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <torque>, defaults to %f",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <commandTopic>, defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <odometryTopic>, defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <odometryFrame>, defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <robotBaseFrame>, defaults to \"%s\"",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <updateRate>, defaults to %f",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <covariance_x>, defaults to %f",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <covariance_y>, defaults to %f",
        ...
        {
            ROS_WARN_NAMED("six_wheel_drive", "GazeboRosSixWheelDrive Plugin (ns = %s) missing <covariance_yaw>, defaults to %f",
        ...
        wheel_speed_[RIGHT_MIDDLE] = 0;
        wheel_speed_[LEFT_MIDDLE] = 0;
        ...
        joints[LEFT_MIDDLE] = this->parent->GetJoint(left_middle_joint_name_);
        joints[RIGHT_MIDDLE] = this->parent->GetJoint(right_middle_joint_name_);

        if (!joints[LEFT_FRONT])
        {
            char error[200];
            snprintf(error, 200,
                     "GazeboRosSixWheelDrive Plugin (ns = %s) couldn't get left front hinge joint named \"%s\"",
                     this->robot_namespace_.c_str(), this->left_front_joint_name_.c_str());
            gzthrow(error);
        }
        ...
        if (!joints[RIGHT_FRONT])
        {
            char error[200];
            snprintf(error, 200,
                     "GazeboRosSixWheelDrive Plugin (ns = %s) couldn't get right front hinge joint named \"%s\"",
                     this->robot_namespace_.c_str(), this->right_front_joint_name_.c_str());
            gzthrow(error);
        }
        ...
        if (!joints[LEFT_REAR])
        {
            char error[200];
            snprintf(error, 200,
                     "GazeboRosSixWheelDrive Plugin (ns = %s) couldn't get left rear hinge joint named \"%s\"",
                     this->robot_namespace_.c_str(), this->left_rear_joint_name_.c_str());
            gzthrow(error);
        }
        ...
        if (!joints[RIGHT_REAR])
        {
            char error[200];
            snprintf(error, 200,
                     "GazeboRosSixWheelDrive Plugin (ns = %s) couldn't get right rear hinge joint named \"%s\"",
                     this->robot_namespace_.c_str(), this->right_rear_joint_name_.c_str());
            gzthrow(error);
        }
        if (!joints[RIGHT_MIDDLE])
        {
            char error[200];
            snprintf(error, 200,
                     "GazeboRosSixWheelDrive Plugin (ns = %s) couldn't get right middle hinge joint named \"%s\"",
                     this->robot_namespace_.c_str(), this->right_middle_joint_name_.c_str());
            gzthrow(error);
        }
        if (!joints[LEFT_MIDDLE])
        {
            char error[200];
            snprintf(error, 200,
                     "GazeboRosSixWheelDrive Plugin (ns = %s) couldn't get left middle hinge joint named \"%s\"",
                     this->robot_namespace_.c_str(), this->left_middle_joint_name_.c_str());
            gzthrow(error);
        }
        ...
        joints[LEFT_MIDDLE]->SetParam("fmax", 0, torque);
        joints[RIGHT_MIDDLE]->SetParam("fmax", 0, torque);
#else
        ...
        joints[LEFT_MIDDLE]->SetMaxForce(0, torque);
        joints[RIGHT_MIDDLE]->SetMaxForce(0, torque);
#endif

        // Make sure the ROS node for Gazebo has already been initialized
        if (!ros::isInitialized())
        {
            ROS_FATAL_STREAM_NAMED("six_wheel_drive", "A ROS node for Gazebo has not been initialized, unable to load plugin. "
        ...
        ROS_INFO_NAMED("six_wheel_drive", "Starting GazeboRosSixWheelDrive Plugin (ns = %s)", this->robot_namespace_.c_str());
        ...
        ros::SubscribeOptions so =
            ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
                                                                boost::bind(&GazeboRosSixWheelDrive::cmdVelCallback, this, _1),
                                                                ros::VoidPtr(), &queue_);
        ...
        this->callback_queue_thread_ =
            boost::thread(boost::bind(&GazeboRosSixWheelDrive::QueueThread, this));

        // listen to the update event (broadcast every simulation iteration)
        this->update_connection_ =
            event::Events::ConnectWorldUpdateBegin(
                boost::bind(&GazeboRosSixWheelDrive::UpdateChild, this));
    }

    // Update the controller
    void GazeboRosSixWheelDrive::UpdateChild()
    {
#ifdef ENABLE_PROFILER
        IGN_PROFILE("GazeboRosSixWheelDrive::UpdateChild");
#endif
#if GAZEBO_MAJOR_VERSION >= 8
        ...
        if (seconds_since_last_update > update_period_)
        {
            ...
            joints[LEFT_MIDDLE]->SetParam("vel", 0, wheel_speed_[LEFT_MIDDLE] / (wheel_diameter_ / 2.0));
            joints[RIGHT_MIDDLE]->SetParam("vel", 0, wheel_speed_[RIGHT_MIDDLE] / (wheel_diameter_ / 2.0));
#else
            ...
            joints[LEFT_MIDDLE]->SetVelocity(0, wheel_speed_[LEFT_MIDDLE] / (wheel_diameter_ / 2.0));
            joints[RIGHT_MIDDLE]->SetVelocity(0, wheel_speed_[RIGHT_MIDDLE] / (wheel_diameter_ / 2.0));
#endif
#ifdef ENABLE_PROFILER
            IGN_PROFILE_END();
#endif
    ...
    void GazeboRosSixWheelDrive::FiniChild()
    {
        alive_ = false;
        queue_.clear();
        queue_.disable();
    ...
    void GazeboRosSixWheelDrive::getWheelVelocities()
    {
        ...
        wheel_speed_[RIGHT_MIDDLE] = vr + va * wheel_separation_ / 2.0;

        wheel_speed_[LEFT_FRONT] = vr - va * wheel_separation_ / 2.0;
        wheel_speed_[LEFT_REAR] = vr - va * wheel_separation_ / 2.0;
        wheel_speed_[LEFT_MIDDLE] = vr - va * wheel_separation_ / 2.0;
    }

    void GazeboRosSixWheelDrive::cmdVelCallback(
    ...
    void GazeboRosSixWheelDrive::QueueThread()
    {
    ...
    void GazeboRosSixWheelDrive::publishOdometry(double step_time)
    {
    ...
    GZ_REGISTER_MODEL_PLUGIN(GazeboRosSixWheelDrive)
}